		...
		if (!is_file(DIR_IMAGE . $image_new) || (filectime(DIR_IMAGE . $image_old) > filectime(DIR_IMAGE . $image_new))) {
			...
			if ($width_orig != $width || $height_orig != $height) {
				...
				if((int)$cheight > $crop_height){
					...
					$image->to_crop($crop_x, ($crop_y - ($crop_y / 2)), $crop_width, $crop_height);
					...
					$image->resizeToHeight($crop_height);
					$image->to_crop(($crop_x - ($crop_x / 2)), $crop_y, $crop_width, $crop_height);
				} else {
					$image->resizeToWidth($crop_width);
				}
				$image->save(DIR_IMAGE . $image_new);
