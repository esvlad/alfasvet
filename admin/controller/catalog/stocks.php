...
class ControllerCatalogStocks extends Controller {
	...
	public function index() {
		$this->language->load('catalog/stocks');
		...
		$total = $this->model_catalog_stocks->getTotalStocks();
		...
		$data['add'] = $this->url->link('catalog/stocks/insert', '&user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('catalog/stocks/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		
		$data['all_stocks'] = array();
		
		$all_stocks = $this->model_catalog_stocks->getAllStocks($filter_data);
		
		foreach ($all_stocks as $stocks) {
			$data['all_stocks'][] = array (
				'stocks_id' 			=> $stocks['stocks_id'],
				'title' 			=> $stocks['title'],
				'short_description'	=> $stocks['short_description'],
				'date_added' 		=> date($this->language->get('date_format_short'), strtotime($stocks['date_added'])),
				'edit' 				=> $this->url->link('catalog/stocks/edit', 'stocks_id=' . $stocks['stocks_id'] . '&user_token=' . $this->session->data['user_token'] . $url, true)
			);
		}
	...
	protected function form() {
		...
		if (isset($this->request->get['stocks_id'])) {
			$data['action'] = $this->url->link('catalog/stocks/edit', '&stocks_id=' . $this->request->get['stocks_id'] . '&user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('catalog/stocks/insert', '&user_token=' . $this->session->data['user_token'], true);
		}
		...
		if (isset($this->request->get['stocks_id'])) {
			$stocks = $this->model_catalog_stocks->getStocks($this->request->get['stocks_id']);
		} else {
			$stocks = array();
		}
		...
		if (isset($this->request->post['stocks_description'])) {
			...
			$data['stocks_description'] = $this->model_catalog_stocks->getStocksDescription($this->request->get['stocks_id']);
		} else {
			$data['stocks_description'] = '';
		}

		if (isset($this->request->post['stocks_seo_url'])) {
			$data['stocks_seo_url'] = $this->request->post['stocks_seo_url'];
		} elseif (isset($this->request->get['stocks_id'])) {
			$data['stocks_seo_url'] = $this->model_catalog_stocks->getStocksSeoUrls($this->request->get['stocks_id']);
		} else {
			$data['stocks_seo_url'] = array();
		}
		
		if (isset($this->request->post['image'])) {
			$data['image'] = $this->request->post['image'];
		} elseif (!empty($stocks)) {
			$data['image'] = $stocks['image'];
		} else {
			$data['image'] = '';
		}
		...
		if (isset($this->request->post['image'])) {
			...
			$data['thumb'] = $this->model_tool_image->resize($stocks['image'] ? $stocks['image'] : 'no_image.png', 100, 100);
		} else {
